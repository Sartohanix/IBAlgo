...
config_url="https://raw.githubusercontent.com/Sartohanix/IBAlgo/refs/heads/master/iba.json"
config_file=""
...
main_dir="$install_dir/iba"
mkdir -p "$main_dir"

# Define the config file path and download it from the GitHub repository
config_file="$main_dir/conf.json"

# Download the config file
echo -n "Downloading config file from $config_url..."
if ! curl -s -o "$config_file" "$config_url"; then
    echo -e "\nError: Failed to download config file from $config_url. Aborting installation."
    return 1
fi
echo " Done."
